...
plt.ylim([min(states[:,0])-minScale, max(states[:,0])+minScale])
plt.title('x')

plt.subplot(4,3,2)
plt.plot(t,states[:,1])
minScale = 1  # Adjust this value according to your desired minimum scale
plt.ylim([min(states[:,1])-minScale, max(states[:,1])+minScale])
plt.title('y')
...
plt.plot(t,states[:,6])
minScale = 1  # Adjust this value according to your desired minimum scale
plt.ylim([min(states[:,6])-minScale, max(states[:,6])+minScale])
plt.title('u')

plt.subplot(4,3,8)
plt.plot(t,states[:,7])
minScale = 1  # Adjust this value according to your desired minimum scale
plt.ylim([min(states[:,7])-minScale, max(states[:,7])+minScale])
plt.title('v')
...
numPoints = len(states[:,1])
# print(f"numPoints: {numPoints}")
# colorMap = np.array([np.linspace(startColor[0], endColor[0], numPoints),
#             np.linspace(startColor[1], endColor[1], numPoints), 
#             np.linspace(startColor[2], endColor[2], numPoints)])

# cm = plt.get_cmap('winter')
# plt.scatter(states[:,0],states[:,1],states[:,2], c=colors)
plt.plot(states[:,0],states[:,1],-1.*states[:,2])
# ax = fig.add_subplot(111, projection='3d')

# for i in range((numPoints)):
#     ax.plot(states[:, 0], states[:, 1], states[:, 2], color=colors[i])

minScale = 1  # Adjust this value according to your desired minimum scale
plt.xlim([min(states[:,0])-minScale, max(states[:,0])+minScale])
plt.ylim([min(states[:,1])-minScale, max(states[:,1])+minScale])
# plt.zlim([min(-1.*states[:,2])-minScale, max(-1.*states[:,2])+minScale])
# grid on
