...
%xlim([167 173]);
xlim([167 172]);
legend('$\omega_x(k)$','$\omega_x(k-1)$','$\omega_{xd}(k)$','$\omega_y(k)$','$\omega_y(k-1)$','$\omega_{yd}(k)$','$\omega_z(k)$','$\omega_z(k-1)$','$\omega_{zd}(k)$','Fault introduced','Location','northeastoutside', 'Interpreter','LaTeX');
xlabel('Sample Number');
ylabel({'$\omega_x(k), \omega_x(k-1), \omega_{xd}(k), \omega_y(k), \omega_y(k-1), \omega_{yd}(k), \omega_z, \omega_z(k-1), \omega_{zd}(k)$ (in deg/s)';'Classifier C1'}, 'Interpreter','LaTeX');
set(gca,'FontSize', 12);
ax = gca().XAxis;
ay = gca().YAxis;
ax.Color = color1;
